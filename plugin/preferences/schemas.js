...
const schema_fence_enhance = () => [
  ...
  Group("buttonGeneral",
    ...
    Switch("SHOW_BUTTON_HINT").ShowIf(DEPS.fenceEnhanceButton),
...
const schema_remote_control = () => [
  Group(
    Switch("ENABLE").ActionTooltip("viewRemoteControlReadme", "fa fa-flask"),
    Text("NAME").Placeholder("defaultIfEmpty"),
  ),
  Group("rpcServer",
    Integer("SERVER_PORT").Min(0).Max(65535).Step(1),
    Password("AUTH_TOKEN").Tooltip("AUTH_TOKEN"),
  ),
  Group("rateLimit",
    Integer("RATE_LIMIT.requestsPerMinute").Min(1),
    Integer("RATE_LIMIT.burstSize").Min(1),
  ),
  Dict("ALLOWED_METHODS").Tooltip("ALLOWED_METHODS"),
  Group(
    Switch("ALLOW_CONFIG_ACCESS").Tooltip("ALLOW_CONFIG_ACCESS"),
    Switch("ENABLE_EVAL").Protect(),
  ),
  FRAG.SettingHandler(),
]
